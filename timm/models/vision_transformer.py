...
from libs.multiway import set_split_position, MultiwayNetwork
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., encoder_dim=None, multiway_kv=False):
        super().__init__()
        ...
        self.kv = nn.Linear(encoder_dim, dim * 2, bias=qkv_bias)
        if multiway_kv:
            self.kv = MultiwayNetwork(self.kv)
    ...
    def forward(self, x, y=None):
        ...
        kv = self.kv(y).reshape(B, Ny, 2, self.num_heads, C // self.num_heads).permute(2, 0, 1, 3, 4)
